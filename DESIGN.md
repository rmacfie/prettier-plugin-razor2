...
Two options control the integration: `csharpierEnabled` (default `true`; when
`false`, C# is kept verbatim) and `csharpierCommand` (default
`dotnet csharpier`). Formatting never throws — every failure falls back to
verbatim C#:
...
- **Disabled** (`csharpierEnabled: false`): silent verbatim, no warning.
